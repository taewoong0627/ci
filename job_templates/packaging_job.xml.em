...
docker run --rm --net=isolated_network -v/etc/flame:/etc/flame --privileged -e GURUMDDS_CONFIG=/etc/flame/gurumdds.yaml -e BUILD_URL="$BUILD_URL" -e UID=`id -u` -e GID=`id -g` -e CI_ARGS="$CI_ARGS" -e CCACHE_DIR=/home/rosbuild/.ccache -i -v `pwd`:`pwd` -v $HOME/.ccache:/home/rosbuild/.ccache $CONTAINER_NAME
echo "# END SECTION"
@[  else]@
